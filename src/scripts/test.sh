...
curl -X GET "$url/calendars/tc1/evaluate?rule=workdays&date=2025-08-14"
echo -e "\n--------------------------------------"
...
curl -X GET "$url/calendars/tc1/evaluate?rule=next_workday&date=2025-03-09"
echo -e "\n--------------------------------------"

echo -e "\n🔹 Get the next workdays after '2025-03-08' and '2025-03-10' ..."
curl -X GET "$url/calendars/tc1/evaluate?rule=next_workday&date=2025-03-08&date=2025-03-10"
echo -e "\n--------------------------------------"

echo -e "\n🔹 Checking if 'invalid' is a  workday..."
curl -X GET "$url/calendars/tc1/evaluate?rule=workdays&date=invalid"
echo -e "\n--------------------------------------"
